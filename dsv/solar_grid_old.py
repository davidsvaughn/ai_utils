...
im = Image.open(f)
# im = im.resize((im1.width//5, im1.height//5), Image.ANTIALIAS)
plt.imshow(im); plt.show()

q = max(im.width, im.height)/640
...
# img = blur(im, k=31, mode=2)
# plt.imshow(img); plt.show()
# save_np(img, fb, False)

# sys.exit()

im = blur(im, k=1, mode=1)
plt.imshow(im); plt.show()
# save_np(im, fc, False)
gray = im.convert('L')
plt.imshow(gray, cmap='Greys_r'); plt.show()

# sys.exit()

g2 = np.array(gray)
plt.imshow(g2, cmap='Greys_r'); plt.show()
save_np(g2, f1)

p = .75 - 0.5*random.random()
g3 = g2*p
q = random.randint(0, int(255-g3.max()))
g3 += q
g3 = 255-g3
g3 = g3.astype(np.uint8)
plt.imshow(g3, cmap='Greys_r'); plt.show()
save_np(g3, f2)


mu,sig = 100,50

g4 = (g2-g2.mean())/g2.std()
g4 = g4*sig + mu
z = np.clip(g4.round(),0,255).astype(np.uint8)
plt.imshow(z, cmap='Greys_r'); plt.show()
save_np(z, f3)

g5 = 255-z
# plt.imshow(g5, cmap='Greys_r'); plt.show()
save_np(g5, f4)

g6 = (g2-g2.mean())/g2.std()
g6 = -g6*sig + mu
z = np.clip(g6.round(),0,255).astype(np.uint8)
plt.imshow(z, cmap='Greys_r'); plt.show()
save_np(z, f5)
...
colormaps = [mpl.cm.viridis,  mpl.cm.viridis_r, mpl.cm.jet_r]#, mpl.cm.jet_r, mpl.cm.coolwarm_r, mpl.cm.rainbow_r]
for colormap in colormaps:
    # colormap = mpl.cm.jet
    cNorm = mpl.colors.Normalize(vmin=0, vmax=255)
    scalarMap = mtpltcm.ScalarMappable(norm=cNorm, cmap=colormap)
    rgba = scalarMap.to_rgba(gray)
    
    rgb = rgba[:,:,:3]
    # rgb = pil2np(contrast(np2pil(rgb), 0.75))
    # plt.imshow(rgb); plt.show()
    save_np(up(rgb), f2, False)
    
    gt = up(rgb2gray(rgb))
    plt.imshow(gt, cmap='Greys_r'); plt.show()
    save_np(gt, f3)
    
    x = (gt-gt.mean())/gt.std()
    mu,sig = 100,50
    x = x*mu + sig
    z = np.clip(x.round(),0,255).astype(np.uint8)
    plt.imshow(z, cmap='Greys_r'); plt.show()
    sys.exit()
