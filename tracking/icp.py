...
def icp(reference_points, points, max_iterations=100, distance_threshold=0.3, convergence_translation_threshold=1e-3,
    ...
    for iter_num in range(max_iterations):
        ...
        M = np.hstack((rot, np.array([[closest_translation_x], [closest_translation_y]])))
        transformation_history.append(M)
        
        M = np.vstack([M,[0,0,1]])
        T = M if T is None else M @ T
    ...
    return T, transformation_history, points
